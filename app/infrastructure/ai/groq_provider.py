"""Adapter Groq para classificação estruturada de e-mails."""
...
from app.infrastructure.ai.prompt import (
    SYSTEM_RULES,
    build_untrusted_email_input,
)
...
def _is_recoverable(exc: BaseException) -> bool:
    """Identifica falhas transitórias que podem receber nova tentativa."""
...
class GroqEmailClassifier:
    """Implementa AIProvider usando a Responses API da Groq."""
    ...
    def model_name(self) -> str:
        """Retorna o modelo configurado sem revelar credenciais."""

        return self._model

    @property
    def prompt_version(self) -> str:
        """Retorna a versão das instruções utilizadas."""
    ...
    async def _retry(
        ...
        """Executa uma operação com tentativas limitadas."""
    ...
    async def classify_email(
        ...
        """Classifica o e-mail usando uma resposta Pydantic estruturada."""
        ...
        try:
            ...
            if not isinstance(
                parsed,
                AIEmailClassificationOutput,
            ):
                raise ValueError("A Groq não retornou classificação estruturada.")
            ...
            raise ExternalServiceError("Groq") from exc
